...
class Store_Apps_Upgrade {
    ...
    function check_for_updates() {
        ...
        if (version_compare( $this->installed_version, $this->live_version, '<=' )) {
            ...
            $check_for_update_url = 'http://www.storeapps.org/wp-admin/admin-ajax.php?action=get_products_latest_version&sku=' . $this->sku . $license_query . '&uuid=' . urlencode( admin_url( '/' ) );
                    ...
                    ?>
                        <tr class="<?php echo $this->prefix; ?>_due_date" style="background: #FFAAAA;">
                            ...
                            <td style="vertical-align: middle;" colspan="2">
                                <?php
                                    if ( $now > $due_date ) {
                                        echo sprintf(__( 'Your license for %s %s. Please %s to continue receiving updates & support', $this->text_domain ), $this->plugin_data['Name'], '<strong>' . __( 'has expired', $this->text_domain ) . '</strong>', '<a href="' . $login_link . '" target="_blank">' . __( 'renew your license now', $this->text_domain ) . '</a>');
                                    } else {
                                        echo sprintf(__( 'Your license for %s %swill expire in %d %s%s. Please %s to get %sdiscount 50%%%s', $this->text_domain ), $this->plugin_data['Name'], '<strong>', $remaining_days, _n( 'day', 'days', $remaining_days, $this->text_domain ), '</strong>', '<a href="' . $login_link . '" target="_blank">' . __( 'renew your license now', $this->text_domain ) . '</a>', '<strong>', '</strong>');
                                    }
                                ?>
                            </td>
                        </tr>
    ...
    function add_support_link( $plugin_meta, $plugin_file, $plugin_data, $status ) {
        ...
        if ( $this->base_name == $plugin_file ) {
            ...
            if ( !empty( $this->documentation_link ) ) {
                $documentation_link = add_query_arg( array( 'utm_source' => $this->sku, 'utm_medium' => 'upgrade', 'utm_campaign' => 'view_docs' ), $this->documentation_link );
                $plugin_meta[] = '<a href="'.$documentation_link.'" target="_blank" title="' . __( 'Documentation', $this->text_domain ) . '">' . __( 'Docs', $this->text_domain ) . '</a>';
            }
